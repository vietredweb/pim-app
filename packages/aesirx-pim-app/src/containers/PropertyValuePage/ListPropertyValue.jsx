...
const ListPropertyValue = observer((props) => {
  const { t } = useTranslation();
  let listSelected = [];
  const viewModel = props.model.propertyValueListViewModel;
  useEffect(() => {
        ...
        return (
          <>
            ...
            <div className="text-green">
              <button
                onClick={() => {
                  historyPush(`/property-value/edit/${value.id}`);
                }}
  ...
  const deletePropertyValues = () => {
    if (listSelected.length < 1) {
      notify(t('txt_row_select_error'), 'error');
    } else {
      viewModel.isLoading();
      viewModel.deletePropertyValues(listSelected);
    }
  };
  ...
  return (
    <div className="px-3 py-4">
      <div className="mb-3 d-flex align-items-center justify-content-between">
        <h2>{t('txt_left_menu_property_value')}</h2>
        ...
        <ActionsBar
          buttons={[
            ...
            },
            {
              ...
              variant: 'success',
              handle: async () => {
                historyPush('/property-value/add');
              },
            },
          ]}
        />
      ...
      <div className="mb-3">
        ...
        >
          <Tab eventKey="default" title={t('txt_all_property_value')} />
        </Tabs>
      </div>
      ...
      <div className="bg-white rounded">
        {viewModel?.successResponse?.state ? (
          <Table
            ...
            data={viewModel?.successResponse?.listPropertyValues}
...
export default withTranslation()(withPropertyValueViewModel(ListPropertyValue));
