...
class PropertyListViewModel {
  ...
  successResponse = {
    ...
    listPropertys: [],
    pagination: null,
    listPropertysWithoutPagination: [],
  };
  ...
  initializeAllData = async () => {
    ...
    const data = await this.propertyStore.getListWithoutPagination(this.successResponse.filters);

    runInAction(() => {
      if (!data?.error) {
        this.callbackOnSuccessGetPropertysHandler(data?.response);
      } else {
        this.onErrorHandler(data?.response);
      }
      this.successResponse.state = true;
  ...
  onSuccessHandler = (result, message) => {
    ...
    if (result?.listItems) {
      this.successResponse.listPropertys = this.transform(result?.listItems);
  ...
  transform = (data) => {
    return data?.map((o) => {
      const date = moment(o[PIM_PROPERTY_DETAIL_FIELD_KEY.MODIFIED_TIME]).format('DD MMM, YYYY');
      return {
        property: {
          id: o[PIM_PROPERTY_DETAIL_FIELD_KEY.ID],
          title: o[PIM_PROPERTY_DETAIL_FIELD_KEY.TITLE],
        },
        lastModified: {
          status: o[PIM_PROPERTY_DETAIL_FIELD_KEY.PUBLISHED],
          dateTime: date ?? '',
          author: o[PIM_PROPERTY_DETAIL_FIELD_KEY.CREATED_USER_NAME],
        },
        published: {
          state: o[PIM_PROPERTY_DETAIL_FIELD_KEY.PUBLISHED],
          id: o[PIM_PROPERTY_DETAIL_FIELD_KEY.ID],
        },
      };
    });
  ...
  deletePropertys = async (arr) => {
    const data = await this.propertyStore.delete(arr);
    runInAction(async () => {
      if (!data?.error) {
  ...
  setPublished = async ({ id, name }, state = 0) => {
    const data = await this.propertyStore.update({
      id: id.toString(),
      property_name: name,
  ...
  callbackOnSuccessGetPropertysHandler = (result) => {
    this.successResponse.listPropertysWithoutPagination = result?.listItems?.map((o) => {
      return {
        value: o?.id,
        label: o[PIM_PROPERTY_DETAIL_FIELD_KEY.TITLE],
      };
    });
  };
...
export default PropertyListViewModel;
