...
const ListVariant = observer((props) => {
  const { t } = useTranslation();
  let listSelected = [];
  const viewModel = props.model.variantListViewModel;
  useEffect(() => {
        ...
        return (
          <>
            ...
            <div className="text-green">
              <button
                onClick={() => {
                  historyPush(`/variant/edit/${value.id}`);
                }}
  ...
  const deleteVariants = () => {
    if (listSelected.length < 1) {
      ...
      viewModel.isLoading();
      viewModel.deleteVariants(listSelected);
    }
  };
  ...
  return (
    <div className="px-3 py-4">
      ...
      <div className="mb-3">
        ...
        >
          <Tab eventKey="default" title={t('txt_all_variant')} />
        </Tabs>
      </div>
      ...
      <div className="bg-white rounded">
        {viewModel?.successResponse?.state ? (
          <Table
            ...
            data={viewModel?.successResponse?.listVariants}
...
export default withTranslation()(withVariantViewModel(ListVariant));
